(() => {
  ...
  function setSoundArmed(armed) {
    soundArmed = armed;
    elements.soundStatus.classList.toggle('is-armed', armed);
    elements.soundStatus.textContent = armed ? 'Sound armed' : 'Sound not armed yet';
  }
  ...
  function ringAlarm() {
    ...
    if (!soundArmed) {
      elements.soundStatus.textContent = 'Trying to play sound';
    }
  ...
  function stopAlarm() {
    ...
    elements.headerTime.textContent = 'HHMM';
    showAlarmControls(false);
  ...
  function cacheElements() {
    ...
    elements.soundStatus = document.getElementById('sound-status');
  ...
  function bindEvents() {
    ...
    elements.confirmAlarm.addEventListener('click', () => {
      armAlarmSound();
      showOverlay(false);
    });
